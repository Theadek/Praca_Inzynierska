...
AnimatedGraphicsObject::AnimatedGraphicsObject(glm::vec3 position, glm::vec3 scale, GLfloat rotate, std::vector<Model*> modelsForWalkAnimation, std::vector<Model*> modelsForJumpAnimation) : GraphicsObject(position, scale, rotate, model) {
    this->lastFrame = 0;
    this->currentAnimationModel = 0;
    this->timeBetweenAnimation = 0;
    this->modelsForWalkAnimation = modelsForWalkAnimation;
    this->modelsForJumpAnimation = modelsForJumpAnimation;
    this->model = modelsForWalkAnimation[0];
}

void AnimatedGraphicsObject::renderModel(Shader* shader) {
    shader->use();
...
void AnimatedGraphicsObject::animateModel(Shader* shader) {
